...
from bzt.utils import humanize_time, is_windows, to_json

import json
import redis

try:
    from bzt.modules.webscreen import HTMLScreen
except ImportError as e:
    print("Import Error: ", e)
    exit(1)


class WebStatusReporter(Reporter, AggregatorListener, Singletone):
    """
    Class to show process status on the web
    :type logger_handlers: list[StreamHandler]
    """

    # NOTE: maybe should use separate thread for screen re-painting
    def __init__(self):
        super(WebStatusReporter, self).__init__()
        self._last_datapoint = None
        self.temp_stream = Cache()
        self.screen = HTMLScreen()
    ...
    def prepare(self):
        ...
        """
        super(WebStatusReporter, self).prepare()
    ...
    def check(self):
        ...
        if self._last_datapoint:
            self.__print_one_line_stats_in_file()
            self._last_datapoint = None
        return False
    ...
    # XXX
    def __print_one_line_stats_in_file(self):
        stats = {
            "source_id": 0,
            "timestamp": 0,
            "current": {
        ...
        stats["source_id"] = self._last_datapoint[DataPoint.SUBRESULTS][0]["id"]
        stats["timestamp"] = self._last_datapoint[DataPoint.TIMESTAMP]
        #stats["data_point"]["subresults"] = to_json(self._last_datapoint[DataPoint.SUBRESULTS])

        cur = self._last_datapoint[DataPoint.CURRENT]['']
        stats["current"]["sample_count"] = cur[KPISet.SAMPLE_COUNT]
        stats["current"]["concurrency"] = cur[KPISet.CONCURRENCY]
        stats["current"]["successes"] = cur[KPISet.SUCCESSES]
        stats["current"]["failures"] = cur[KPISet.FAILURES]
        stats["current"]["avg_resp_time"] = cur[KPISet.AVG_RESP_TIME]
        stats["current"]["stdev_resp_time"] = cur[KPISet.STDEV_RESP_TIME]
        stats["current"]["avg_latency"] = cur[KPISet.AVG_LATENCY]
        stats["current"]["avg_conn_time"] = cur[KPISet.AVG_CONN_TIME]
        stats["current"]["byte_count"] = cur[KPISet.BYTE_COUNT]
        stats["current"]["errors"] = cur[KPISet.ERRORS]
        #stats["data_point"]["current"]["resp_times"] = to_json(cur[KPISet.RESP_TIMES])
        stats["current"]["resp_codes"] = cur[KPISet.RESP_CODES]
        stats["current"]["percentiles"] = cur[KPISet.PERCENTILES]

        cumul = self._last_datapoint[DataPoint.CUMULATIVE]['']
        stats["cumulative"]["sample_count"] = cumul[KPISet.SAMPLE_COUNT]
        stats["cumulative"]["concurrency"] = cumul[KPISet.CONCURRENCY]
        stats["cumulative"]["successes"] = cumul[KPISet.SUCCESSES]
        stats["cumulative"]["failures"] = cumul[KPISet.FAILURES]
        stats["cumulative"]["avg_resp_time"] = cumul[KPISet.AVG_RESP_TIME]
        stats["cumulative"]["stdev_resp_time"] = cumul[KPISet.STDEV_RESP_TIME]
        stats["cumulative"]["avg_latency"] = cumul[KPISet.AVG_LATENCY]
        stats["cumulative"]["avg_conn_time"] = cumul[KPISet.AVG_CONN_TIME]
        stats["cumulative"]["byte_count"] = cumul[KPISet.BYTE_COUNT]
        stats["cumulative"]["errors"] = cumul[KPISet.ERRORS]
        #stats["data_point"]["cumulative"]["resp_times"] = to_json(cumul[KPISet.RESP_TIMES])
        stats["cumulative"]["resp_codes"] = cumul[KPISet.RESP_CODES]
        stats["cumulative"]["percentiles"] = cumul[KPISet.PERCENTILES]
        stats["cumulative"]["plr"] = 100 * (cumul[KPISet.FAILURES] / cumul[KPISet.SAMPLE_COUNT])

        result = json.dumps(stats, indent=0)
        if (self.__b_lines):
            result =  ',\n' + result
        else:
            self.__b_lines = True
    ...
    def startup(self):
        super(WebStatusReporter, self).startup()
    ...
    def shutdown(self):
        ...
        # XXX
        end_str = ']}'
        self.file_out.write(end_str)
        self.file_out.close()

        super(WebStatusReporter, self).shutdown()
        if self.disabled:
            return
    ...
    def post_process(self):
        super(WebStatusReporter, self).post_process()
        self.__dump_saved_log()
